"""
Deploy: week filter + periodo fix + threads backfill.
Git pull, restart service, then run threads backfill.
"""
import paramiko
import sys
import io
...
with Session() as db:
    ...
    print("token inicio:", access_token[:30] if access_token else "VACIO")
    ...
    try:
        ...
        print("Posts API:", len(items))
        print("Paging:", list(paging.keys()))
        for it in items[:5]:
    ...
    last = db.query(Publicacion).filter(
        Publicacion.medio_id == medio.id,
        Publicacion.canal == CanalEnum.threads,
    ).order_by(Publicacion.fecha_publicacion.desc()).first()
    if last:
        print("Ultimo threads en BD:", last.fecha_publicacion, repr((last.texto or "")[:80]))
...
with Session() as db:
    medio = db.query(Medio).filter(Medio.slug == SLUG).first()
    if not medio:
        log.error("Medio no encontrado: %s", SLUG)
        sys.exit(1)

    # Reset checkpoint de threads
...
log.info("Ejecutando detect_new de threads desde %s ...", fecha_desde.date())
with Session() as agent_db:
    try:
        ...
        log.info("threads: nuevas=%d actualizadas=%d", nuevas, actualizadas)
    except Exception as ex:
        log.error("ERROR threads: %s", ex)
        import traceback
        traceback.print_exc()
        try:
            agent_db.rollback()
...
def main():
    ...
    sys.stdout.buffer.write(b"Conectado OK\n"); sys.stdout.buffer.flush()

    # 1. Git pull
    run(ssh, f"cd {PROJECT} && git pull origin main 2>&1", label="git pull")

    # 2. Rebuild frontend (si hay node_modules)
    run(ssh, f"cd {PROJECT}/frontend && npm run build 2>&1 || echo 'build skipped'",
        timeout=120, label="frontend build")
    ...
    # 4. Restart servicio
    run(ssh, "systemctl restart social-intelligence && sleep 4 && systemctl is-active social-intelligence",
        label="restart servicio")

    # 5. Diagnostico Threads
    run(ssh, f"cd {PROJECT} && {VENV} /tmp/_threads_diag.py 2>&1",
        timeout=60, label="DIAGNOSTICO THREADS")

    # 6. Backfill Threads desde 2026-01-01
